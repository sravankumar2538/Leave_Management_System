...
@Component({
  ...
  standalone: true, // Add standalone: true if it's meant to be a standalone component (missing in original)
  imports: [CommonModule],
  templateUrl: './holiday-calendar.component.html',
  styleUrl: './holiday-calendar.component.css'
})
export class HolidayCalendarComponent implements OnInit, OnDestroy { // Implement OnDestroy
  ...
  getHolidayCalendar(): void {
    ...
    this.holidayCalendarService.UpcomingHolidays()
        ...
        next: (response: ApiResponse<HolidayCalendarResponseDto[]>) => { // Correctly expecting an array
          ...
          if (response.isSuccess) {
            if (response.data && response.data.length > 0) {
              ...
              this.errorMessage = 'No holiday data found.';
              this.holidayCalendarData = []; // Ensure data is cleared
            }
          } else {
            // Handle cases where isSuccess is false (backend returned an error message)
            this.hasError = true;
            this.errorMessage = response.message || 'Failed to retrieve Upcomming Holidays.'; // Using response.message
            this.holidayCalendarData = []; // Ensure data is cleared
          }
        },
